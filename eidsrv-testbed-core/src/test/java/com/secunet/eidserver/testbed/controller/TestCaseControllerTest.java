...
{
	TestCaseController testCaseController = new TestCaseControllerBean();
	...
	@Test
	public void testModuleFunctionality() throws Exception
	{
		// setup
		TestCandidate candidate = new TestCandidateTestingPojo();
		...
		TestCase secondTc = new CopyTestCaseTestingPojo();
		List<String> module2 = new ArrayList<>();
		module2.add("FOO");
		module2.add("BAZ");
		module2.add("42");
		secondTc.setModule(module2);

		Set<TestCase> testCases = new HashSet<>();
		testCases.add(tc);
		testCases.add(secondTc);
		candidate.setTestCases(testCases);

		// test actual functionality
		TestModule root = testCaseController.getRootTestModule(candidate);
		assertNotNull(root);
		assertNotNull(root.getSubModules());
		TestModule foo = root.getSubModule("FOO");
		assertNotNull(foo);
		assertNotNull(foo.getSubModules());
		TestModule bar = foo.getSubModule("BAR");
		assertNotNull(bar);
		assertNotNull(bar.getSubModules());
		TestModule baz = foo.getSubModule("BAZ");
		assertNotNull(baz);
		assertNotNull(baz.getSubModules());
		TestModule answer = bar.getSubModule("42");
		assertNotNull(answer);
		assertNotNull(answer.getTestCases());
		assertTrue(answer.getTestCases().contains(tc));
		TestModule secondAnswer = baz.getSubModule("42");
		assertNotNull(secondAnswer);
		assertNotNull(secondAnswer.getTestCases());
		assertTrue(secondAnswer.getTestCases().contains(secondTc));
	}
