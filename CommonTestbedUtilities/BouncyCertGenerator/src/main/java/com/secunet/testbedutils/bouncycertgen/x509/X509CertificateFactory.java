...
import com.secunet.testbedutils.bouncycertgen.xml.x509.BasicConstraintsType;
import com.secunet.testbedutils.bouncycertgen.xml.x509.CRLDistributionPointsType.DistributionPoint;
import com.secunet.testbedutils.bouncycertgen.xml.x509.CertificateDefinition;
import com.secunet.testbedutils.bouncycertgen.xml.x509.DistributionPointType;
import com.secunet.testbedutils.bouncycertgen.xml.x509.GeneralNameTypeType;
import com.secunet.testbedutils.bouncycertgen.xml.x509.GeneralNamesType;
...
public class X509CertificateFactory
{
	 ...
	 *
	 * @param {@link
	 * 			CertificateDefinition} certificateDefinition - The
	 *            definition file for the certificate
	 * @return {@link GeneratedCertificate}
	 */
	 ...
	 *            definition file for the certificate
	 * @param {@link
	 * 			GeneratedCertificate} signer - Signer of the certificate
	 ...
	 *            definition file for the certificate
	 * @param relativeDate
	 *            - Relative date of certificate (if null, current date
	 *            will be used)
	 * @return {@link CertKeysPair}
	 */
	...
	public static GeneratedCertificate createX509(CertificateDefinition certificateDefinition, GeneratedCertificate signer, Date relativeDate)
	{
		...
		if (pair == null)
		{
			...
			if (certificateDefinition.getKeyAlgorithm() != null)
			{
				if (certificateDefinition.getKeyAlgorithm().getECDSA() != null)
				{
					keyType = AlgorithmID.ECDSA;
					curveName = certificateDefinition.getKeyAlgorithm().getECDSA().value();
				}
				else if (certificateDefinition.getKeyAlgorithm().getRSA() != null)
				{
					keyType = AlgorithmID.RSA;
					keyLength = certificateDefinition.getKeyAlgorithm().getRSA();
				}
				else if (certificateDefinition.getKeyAlgorithm().getDSA() != null)
				{
					keyType = AlgorithmID.DSA;
					keyLength = certificateDefinition.getKeyAlgorithm().getDSA();
				}
			}
			else
			{
		...
		{
			signpair = pair;
			// was the signaureAlgorithm extension provided?
			if (null != certificateDefinition.getSignatureAlgorithm())
			{
				if (certificateDefinition.getSignatureAlgorithm().getECDSA() != null)
				{
					signatureAlgorithm = certificateDefinition.getSignatureAlgorithm().getECDSA().value();
				}
				else if (certificateDefinition.getSignatureAlgorithm().getRSA() != null)
				{
					signatureAlgorithm = certificateDefinition.getSignatureAlgorithm().getRSA().value();
				}
				else if (certificateDefinition.getSignatureAlgorithm().getDSA() != null)
				{
					signatureAlgorithm = certificateDefinition.getSignatureAlgorithm().getDSA().value();
				}
			}
			else
			{
				// no extension provided, fall back to the defaults
				switch (AlgorithmID.valueOf(signpair.getPrivate().getAlgorithm()))
				{
					case ECDSA:
						signatureAlgorithm = SignatureAlgorithmTypeECDSA.SHA_1_WITH_ECDSA.value();
						break;
					case RSA:
						signatureAlgorithm = SignatureAlgorithmTypeRSA.SHA_1_WITH_RSA.value();
						break;
					case DSA:
						signatureAlgorithm = SignatureAlgorithmTypeDSA.SHA_1_WITH_DSA.value();
						break;
				}
			}
		}
			...
			try
			{
				...
				else
				{
					logger.log(Level.SEVERE,
							"The provided key file format for the signature could not be recognized. Filename: " + keyFile.getAbsolutePath() + ". A new key pair will be generated instead.");
				}
			}
			catch (IOException e)
			{
	 ...
	 *
	 * @param {@link
	 * 			CertificateDefinition} definition - The definition file for
	 *            the certificate
	 * @param {@link
	 * 			X509v3CertificateBuilder} builder - The certificate builder
	 *            instance
	 */
	 ...
	 *
	 * @param {@link
	 * 			Object} x509extensionType The extension object on which to
	 *            work
	 * @param {@link
	 * 			Class<?>} bcClass The class of the x509extensionType object
	...
	private static Set<Object> buildObjectExtension(Object x509extensionType, Class<?> bcClass)
	{
		...
		for (Method m : x509extensionType.getClass().getDeclaredMethods())
		{
			...
			catch (IllegalArgumentException e)
			{
				StringWriter trace = new StringWriter();
				e.printStackTrace(new PrintWriter(trace));
				logger.log(Level.WARNING, "Could not load x.509 extension: Illegal Argument Exception." + System.getProperty("line.separator") + trace.toString());
			}
			catch (InvocationTargetException e)
			{
				StringWriter trace = new StringWriter();
				e.printStackTrace(new PrintWriter(trace));
				logger.log(Level.WARNING, "Could not load x.509 extension: Invocation Target Exception." + System.getProperty("line.separator") + trace.toString());
			}
		}
		return objectSet;
	}
	 ...
	 *
	 * @param {@link
	 * 			Method} m The method to work on
	 ...
	 *
	 * @param {@link
	 * 			CertificateDefinition} input The certificate definition XML
	 *            to parse
	 * @param {@link
