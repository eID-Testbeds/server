...
import com.secunet.testbedutils.cvc.cvcertificate.Oids;
...
import com.secunet.testbedutils.cvc.cvcertificate.TLV;
...
public class CVCertGen {
                    ...
                    case "keys": // checking/generating keys
                        ...
                        for (int j = 0; j < keysChilds.getLength(); j++) {
                            if (keysChilds.item(j).getNodeName().equals("key")) {
                                KeyInfo key = null;
                                try {
                                    key = generateKeyInfoFromNode(keysChilds.item(j), save);
                                } catch (Exception e) {
                                    logger.warn(e.getMessage());
                                }
     ...
     * @throws Exception
     */
    protected KeyInfo generateKeyInfoFromNode(Node node, boolean save) throws Exception {
        ...
        if (keyInfo != null) {
            ...
            if (create) {
                // generating key pair
                KeyPairGenerator keyGen = KeyPairGenerator.getInstance(keyInfo.getType(), "BC");
                ...
                if (save) {
                    // saving keys
                    File privKeyFile = new File(baseDir, keyInfo.getFilePrivateKey());
                    ...
                    if (keyInfo.getFilePublicKey() != null) {
                        File pubKeyFile = new File(baseDir, keyInfo.getFilePublicKey());
                        ...
                        File pubKeyCVFile = new File(baseDir, keyInfo.getFilePublicKey() + ".cv");
                        ...
                        pubKeyCV.writeToFile(pubKeyCVFile.getAbsolutePath());
                    }
                ...
                // loading keys
                KeyFactory kf = KeyFactory.getInstance(keyInfo.getType(), "BC");
                ...
                if (privKeyFile.exists()) {
                    ...
                    if (dbKey != null) {
                        ...
                        logger.warn("Unable to load private key of " + keyInfo.getName() + ".");
                        return null;
                    }
                } else {
                    logger.warn("Private key of " + keyInfo.getName() + " does not exist.");
                    return null;
                }
                ...
                if (keyInfo.getFilePublicKey() != null) {
                    ...
                    if (pubKeyFile.exists()) {
                        ...
                        if (dbKey != null) {
                            ...
                            logger.warn("Unable to load public key of " + keyInfo.getName() + ".");
                            return null;
                        }
                    } else {
                        logger.warn("Public key of " + keyInfo.getName() + " does not exist.");
                        return null;
                    }
                } else {
                    pubKey = null;
                ...
                keyInfo.setKeyPair(new KeyPair(pubKey, privKey));
            }
        }
    ...
    protected LinkedList<DataBuffer> getSectorKeysFromNode(Node node) throws Exception {
        LinkedList<DataBuffer> out = new LinkedList<DataBuffer>();
            ...
            switch (terminalSectorChilds.item(i).getNodeName()) {
                case "fileSectorPublicKey": {
                    ...
                    if (terminalSectorFile.exists()) {
                        try {
                            out.add(DataBuffer.readFromFile(terminalSectorFile.getAbsolutePath()));
                        } catch (IOException e) {
